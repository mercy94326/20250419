  ...
  for (let i = 0; i < buttonNames.length; i++) {
    ...
    if (buttonNames[i] === "作品集") {
      btn.mousePressed(() => {
        ...
        let projectNames = ["專案 1", "專案 2", "專案 3"];
        let projectLinks = [
      ...
      btn.mousePressed(() => {
        ...
        if (buttonNames[i] === "測驗卷") {
          ...
          imgContainer.style("pointer-events", "none"); // 確保容器不干擾按鈕點擊

          const img = createImg("456.PNG");
          img.id("selfIntroImage"); // 設置圖片的 ID，方便切換
          img.style("width", "400px"); // 縮小圖片寬度為 400px
          img.style("height", "auto"); // 自動調整高度以保持比例
          img.style("object-fit", "contain"); // 確保圖片保持比例
          img.style("cursor", "pointer"); // 設置圖片為可點擊
          img.style("pointer-events", "auto"); // 確保圖片本身可以接收滑鼠事件
          img.style("z-index", "1001"); // 確保圖片在容器內，但不遮擋按鈕

          // 添加滑鼠移入事件，切換為圖檔 123.PNG
          img.mouseOver(() => {
            img.attribute("src", "123.PNG"); // 切換為圖檔 123.PNG
          });

          // 添加滑鼠移出事件，切換回圖檔 456.PNG
          img.mouseOut(() => {
            img.attribute("src", "456.PNG"); // 切換回圖檔 456.PNG
          });
